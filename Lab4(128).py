...
# m = int(input("Введіть кількість повторів кожної комбінації: "))
m = 3
...
Disp8 = 0
for i in range(m):
    Disp1 += ((Y_ROWS[0][i] - Y_ROWS_AV[0]) ** 2) / m
    Disp2 += ((Y_ROWS[1][i] - Y_ROWS_AV[1]) ** 2) / m
    Disp3 += ((Y_ROWS[2][i] - Y_ROWS_AV[2]) ** 2) / m
    Disp4 += ((Y_ROWS[3][i] - Y_ROWS_AV[3]) ** 2) / m
    Disp5 += ((Y_ROWS[4][i] - Y_ROWS_AV[4]) ** 2) / m
    Disp6 += ((Y_ROWS[5][i] - Y_ROWS_AV[5]) ** 2) / m
    Disp7 += ((Y_ROWS[6][i] - Y_ROWS_AV[6]) ** 2) / m
    Disp8 += ((Y_ROWS[7][i] - Y_ROWS_AV[7]) ** 2) / m

sum_dispersion = Disp1 + Disp2 + Disp3 + Disp4 + Disp5 + Disp6 + Disp7 + Disp8
DISPS = [Disp1, Disp2, Disp3, Disp4, Disp5, Disp6, Disp7, Disp8]
...
if Gp < Gt:
    ...
    print("Дисперсія не однонідна. Збільшіть m")
